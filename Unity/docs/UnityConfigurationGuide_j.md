[和訳]
# Unity 設定ガイド（Configuration Guide）

## C規格、コンパイラ、マイコンについて

組み込みソフトウェアの世界には多くの課題があります。  
コンパイラはC標準の異なるバージョンをサポートしており、  
時には特定の目的のために標準の要件を無視することもあります。  
また、サポートを簡素化するためや、ターゲットとするマイコン特有の癖による場合もあります。  
シミュレータもこの多様性にさらなる複雑さを加えます。

Unityは、Cコンパイラがターゲットとするほぼすべての環境で動作するよう設計されています。  
もし何の設定もせずに動作すれば理想的ですが、残念ながらすべてのターゲットでそれが実現できるわけではありません。  
このドキュメントで説明する設定オプションのうち、少なくともいくつかは必要になる可能性が高いです。

Unityのすべての設定オプションは `#define` で指定します。  
ほとんどは単純な定義ですが、一部は引数付きのマクロです。  
これらは `unity_internals.h` ヘッダファイル内に記述されています。  
本当に必要な場合を除き、このファイルを直接編集することは推奨しません。  
このファイルはクロスプラットフォームライブラリの構築がいかに難しいかを示す証拠でもあります。  
しかし、逆に言えば、複雑さを1か所に集約することで、他の部分をより一貫性がありシンプルにできるという証明でもあります。

---

### これらのオプションの使い方

ターゲット専用のコンパイラやシミュレータ、あるいはネイティブコンパイラを使っている場合でも、  
これらの設定方法にはいくつか選択肢があります。

1. **コンパイラのコマンドラインフラグで指定する**  
   これらのオプションはCのdefineで指定するため、  
   ほとんどのオプションはコンパイラのコマンドライン引数として渡すことができます。  
   組み込みターゲットで、すべての設定をIDEで行う必要がある場合でも、  
   プロジェクト内のどこかにコンパイラ用のdefineを設定する場所が必ずあります。

2. **カスタムの `unity_config.h` 設定ファイルを作成する**  
   このファイルをツールチェーンの検索パス内に配置します。  
   このファイルには、ターゲット固有のdefineやマクロを記述します。  
   必要なのは `UNITY_INCLUDE_CONFIG_H` をdefineすることだけです。  
   これにより、Unityは以降のdefineを `unity_config.h` から取得するようになります。

> 注意：  
> これらのdefineをテストファイル内で直接記述してもうまく動作しない場合が多いです。  
> これらのdefineは、`unity.h` がインクルードされるすべての場所（テスト本体、テストランナー、unity.cのビルド時）で有効である必要があります。
...
C言語の開発経験が長い方ならご存知かもしれませんが、  
C言語における整数型の概念はターゲットごとに異なります。  
C標準では、`int` 型がターゲットマイクロプロセッサのレジスタサイズに一致することや、  
`int` のサイズが他の整数型とどう関係するかについて規定があります。  
あるターゲットでは `int` が16ビット、別のターゲットでは64ビットということもあります。

C99以降に準拠したコンパイラではより具体的な型が用意されていますが、  
すべてのコンパイラがそれに対応しているわけではありません。  
そのため、Unityには必要な整数サイズに合わせて調整できる機能がいくつか用意されています。  
まずは自動的に最適な型を推測しようとします。

---

#### `UNITY_EXCLUDE_STDINT_H`

Unityが型を推測する際、最初に行うのは `stdint.h` の確認です。  
このファイルには `UINT_MAX` などのdefineが含まれており、  
Unityはこれを利用してシステムの情報を取得します。

ただし、これを利用したくない場合（なぜでしょう？）や、  
より一般的には、システムが `stdint.h` をサポートしていない場合もあります。  
その場合は、このマクロをdefineしてください。  
これにより、Unityはこのファイルのインクルードをスキップし、  
コンパイラエラーを回避できます。
...
型の自動推測の2番目の手段として、Unityは `limits.h` をチェックします。  
一部のコンパイラは `stdint.h` をサポートしていなくても、代わりに `limits.h` を含んでいる場合があります。  
Unityにこのファイルもチェックさせたくない場合は、このマクロを定義してインクルードをスキップさせてください。
...
上記2つの自動オプション（UNITY_EXCLUDE_STDINT_H と UNITY_EXCLUDE_LIMITS_H）の両方を無効にした場合は、
自分で型の設定を行う必要があります。
心配はいりません。
デフォルトが気に入らない場合でも、指定すべきdefineはほんの数個だけです。

#### `UNITY_INT_WIDTH`

このマクロには、あなたのシステムで int 型が占めるビット数を定義します。
自動検出できない場合のデフォルト値は32ビットです。
...
このマクロには、あなたのシステムで long 型が占めるビット数を定義します。
自動検出できない場合のデフォルト値は32ビットです。
この値は、システムがどのような64ビットサポートを持っているかを判断するために使われます。
64ビット値を得るために long か long long のどちらを使うべきかを決める際に利用されます。
16ビットシステムでは、このオプションは無視されます。
...
このマクロには、あなたのシステムでポインタが占めるビット数を定義します。
自動検出できない場合のデフォルト値は32ビットです。
ポインタのキャストに関するコンパイラ警告が出る場合は、この設定を見直してください。

_ヒント:_
特殊なプロセッサ（例：TI C55x のような23ビット幅のポインタを持つもの）をサポートするためには、
次に大きい2のべき乗（この場合は32ビット）を選択してください。

_サポートされる値:_ 16, 32, 64

_例:_

```C
// 以下のいずれかの #define でポインタ幅を設定します（自動検出されない場合）
//#define UNITY_POINTER_WIDTH 16
//#define UNITY_POINTER_WIDTH 32
#define UNITY_POINTER_WIDTH 64 // ポインタ幅を64ビットに設定
```

#### `UNITY_COMPARE_PTRS_ON_ZERO_ARRAY`

このマクロを定義すると、配列アサーションで長さが0の場合、内容ではなくポインタ同士を比較するようになります。  
有効にしない場合、長さ0を指定すると常に失敗となり、「空の配列を比較しようとしました」という警告メッセージが表示されます。

---

#### `UNITY_SUPPORT_64`

Unityは、自動検出で64ビットサポートが必要と判断した場合（`int`、`long`、またはポインタ幅が32ビットを超える場合）には自動的に64ビットサポートを有効にします。  
もし他のオプションで自動的に有効にならない場合、このマクロを定義することで64ビットサポートを強制的に有効化できます。  
ただし、小規模なターゲットで64ビットサポートを有効にすると、サイズや速度に大きな影響が出る場合があるため、必要がなければ定義しないでください。
...
組み込みの世界では、ターゲットが浮動小数点演算を全くサポートしていなかったり、単精度（float）のみ対応していることが珍しくありません。
整数型は必ず何らかのサイズで利用できますが、浮動小数点型は全く利用できない場合もあります。
このようなプラットフォームで float.h をインクルードしようとするとエラーになるため、手動での設定が唯一の方法となります。
...
デフォルトでは、Unityは単精度浮動小数点（float）サポートを有効、倍精度（double）は無効と推測します。
これらのマクロを使うことで、float/doubleのサポートを明示的に有効・無効にできます。
どちらも含めない、どちらか一方のみ、または両方を含めることも可能です。
有効化した場合、以下の浮動小数点関連オプションも利用可能になります。
